...
source "$script_dir/DE4_QSYS_mem_if_ddr2_emif_p0_report_timing_core.tcl"
...
foreach inst $instances {
	...
	if {[llength $instances] <= 1} {
		set fbasename "${::GLOBAL_DE4_QSYS_mem_if_ddr2_emif_p0_corename}"
	} else {
		set fbasename "${::GLOBAL_DE4_QSYS_mem_if_ddr2_emif_p0_corename}_${inst_id}"
	}
	...
	foreach opc $opcs {
		...
		if {!$in_gui && [array exists assumptions_cache] &&  [info exists assumptions_cache(${::GLOBAL_DE4_QSYS_mem_if_ddr2_emif_p0_corename}-$inst)] } {
			set assumptions_valid $assumptions_cache(${::GLOBAL_DE4_QSYS_mem_if_ddr2_emif_p0_corename}-$inst)
